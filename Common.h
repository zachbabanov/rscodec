...
//#define DEBUG_IN_RELEASE

#if defined(_DEBUG) || defined(DEBUG) || defined(DEBUG_IN_RELEASE)
    #define DEBUG
    #ifdef _WIN32
    ...
    #endif
    #define DEBUG_ASSERT(cond) { if (!(cond)) { DEBUG_BREAK; } }
#else
    #define DEBUG_BREAK ;
...
// Platform/Architecture
    ...
    #define ALIGNED_ACCESSES /* Inputs must be aligned to ALIGN_BYTES */
...
// Precondition: x != 0
FORCE_INLINE unsigned LastNonzeroBit32(unsigned x)
{
#ifdef _MSC_VER
    unsigned long index;
...
FORCE_INLINE unsigned NextPow2(unsigned n)
{
...
public:
    ...
    FORCE_INLINE void Initialize(void* dest)
    {
        DestBuffer = dest;
        Waiting = nullptr;
    }
    ...
    FORCE_INLINE void Add(const void* src, const uint64_t bytes)
    {
#ifdef M1_OPT
        if (Waiting)
        {
    ...
    FORCE_INLINE void Finalize(const uint64_t bytes)
    {
#ifdef M1_OPT
        if (Waiting)
...
static FORCE_INLINE uint8_t* SIMDSafeAllocate(size_t size)
{
...
static FORCE_INLINE void SIMDSafeFree(void* ptr)
{
    if (!ptr)
        return;
