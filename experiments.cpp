...
//#define SHORT_FIELD
//#define EXPERIMENT_EXTRA_XOR
//#define EXPERIMENT_EXTRA_MULS
#define EXPERIMENT_CANTOR_BASIS
...
#define FWHT_OPTIMIZED

typedef ffe_t fwht_t;

// {a, b} = {a + b, a - b} (Mod Q)
static FORCE_INLINE void FWHT_2(fwht_t& RESTRICT a, fwht_t& RESTRICT b)
{
...
static FORCE_INLINE void ifft_butterfly(ffe_t& a, ffe_t& b, ffe_t skew)
{
    b ^= a;
    if (skew != kModulus)
        a ^= mulE(b, skew);
...
static FORCE_INLINE void fft_butterfly(ffe_t& a, ffe_t& b, ffe_t skew)
{
    if (skew != kModulus)
        a ^= mulE(b, skew);
    b ^= a;
...
// Precondition: x != 0
FORCE_INLINE unsigned LastNonzeroBit64(uint64_t x)
{
#ifdef _MSC_VER
#ifdef _WIN64
    unsigned long index;
